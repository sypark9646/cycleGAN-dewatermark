...
class cyclegan():
    ...
    def train(self):
        ...
        self.g_optim = tf.train.AdamOptimizer(self.lr_var*5, beta1=self.beta1) \
        ...
        if self.continue_train:
            ...
            if suc_or_fal:
                ...
                counter = int(num_of_train)                   # 이어서 저장하기 위해서..
            else:
        ...
        for epoch in range(self.epoch):
            ...
            for idx in range(0, batch_idxs):
                ...
                counter += 1
                #print(tf.reduce_mean(tf.reduce_min(tf.stack(self.d_loss), 0)), tf.reduce_mean(tf.reduce_min(tf.stack(self.g_loss), 0)))
...
def main():
    checkpoint_dir = './checkpoint/watermark'             # 체크포인트 경로
    test_dir = './datasets/face/testA'                          # 테스트 이미지가 저장되는 경로
    dataset_dir = './datasets/face'                         # 데이터셋 위치    trainA trainB testA testB
    phase = "train"  # or test
    which_direction = "AtoB"        # or BtoA .  테스트시 변환 방향
